...
//   panic! macro (used by result.expect/unwrap)
//   should be the last resort; not in a library
// recoverable errors
//   handle
//     if let or match expression
//     many already in result
//   return the error
//     match expression -> simplified syntax
//     `let file = File::open("myfyle")?`
//     by default works with same type error messages
//     with anyhow package you can use the Result from their that handles
//       any type that implement the Error trait
...
fn play_time(dolphin: &Dolphin) -> Result<Vec<String>> {
    ...
    let mut response = dolphin.say_your_name()?;
    responses.push(response);

    response = dolphin.flip()?;
    responses.push(response);
...
//fn main() -> Result<()> {
    //...
    //for dolphin in &dolphins {
        //play_time(dolphin).context("dolphin can't play...")?;
    //}
    //Ok(())
//}
